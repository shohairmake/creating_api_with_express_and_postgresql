...
describe(chalk.green('test_POST_/api/todos'), () => {
    ...
    it('400 error if title is missing', async () => {
        ...
        assert.strictEqual(response.body.error.message, 'null value in column "title" violates not-null constraint');
    });
    it('400 error if body is missing', async () => {
        ...
        assert.strictEqual(response.body.error.message, 'null value in column "body" violates not-null constraint');
    });
});